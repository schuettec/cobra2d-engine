...
public class Animation {
	...
	public Animation(final VolatileImage image, final int width, final int height) {
		this.width = width;
		this.height = height;
	...
	private void setIndex(final int index) throws IllegalArgumentException {
		...
		if (index < 0 || index >= (this.getPictureCount())) {
			throw new IllegalArgumentException("Picture-Index out of bounds. \n Min-Index: 0 \n Max-Index: "
	...
	public VolatileImage getImage(final int index) {
		...
		final VolatileImage tmpImage = RenderToolkit.createVolatileImage(this.width, this.height);
		final Graphics2D g = (Graphics2D) tmpImage.getGraphics();
		g.drawImage(this.image, 0, 0, this.width, this.height, this.rect.x, this.rect.y, this.rect.x + this.width,
		    this.rect.y + this.height, null);

		if (this.filter != null) {
			return RenderToolkit.convertSpriteToTransparentSprite(tmpImage, this.filter);
		}
