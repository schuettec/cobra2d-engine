...
public interface AbstractCamera extends Camera {
  ...
  default void drawPoint(RendererAccess renderer, Point point,
      int radius, Color color) {
    renderer.fillCircle(point.getRoundX(), point.getRoundY(),
        radius, color);
  }

  default List<Collision> sortByLayer(
      List<Collision> capturedEntities) {
    Comparator<Renderable> layerComparator = Comparator
        .comparingInt(Renderable::getLayer);
    Comparator<Collision> collisionComparator = Comparator
        .comparing(Collision::getOpponent, (e1, e2) -> {
          if (e1 instanceof Renderable
              && e2 instanceof Renderable) {
  ...
  @Override
  default void render(final RendererAccess renderer,
      Cobra2DWorld map, List<Collision> capturedEntities) {
    ...
    for (Collision collision : capturedEntities) {
      ...
      getCollisionPoints(collisionPoints, renderer, map,
          worldToScreenTranslation, entity);
      drawEntityPoint(renderer, worldToScreenTranslation,
          entity);
    }
  ...
  default void centerOnScreen(final RendererAccess renderer) {
    if (isCenterOnScreen()) {
      setScreenPosition(new Point(renderer.getWidth() / 2.0,
  ...
  default Point getHalfDimensionTranslation() {
    ...
    return new Point(cameraDimension.getWidth() / 2.0,
        cameraDimension.getHeight() / 2.0);
  }

  @Override
  default Point screenToWorldCoordinates(
  ...
  default void drawCollisionShape(final RendererAccess renderer,
      Shape entityShape) {
    if (isDrawCollisionShape() && entityShape.isPointBased()) {
      ...
      for (Point p : entityShape.getPoints()) {
        ...
        if (isNull(lastPoint)) {
          ...
          renderer.drawLine(lastPoint.getRoundX(),
              lastPoint.getRoundY(), p.getRoundX(),
              p.getRoundY(), Color.YELLOW);
          lastPoint = p;
        }
      }

      renderer.drawLine(firstPoint.getRoundX(),
          firstPoint.getRoundY(), lastPoint.getRoundX(),
          lastPoint.getRoundY(), Color.YELLOW);
    }
  }
  ...
  default void drawCollisionPoints(final RendererAccess renderer,
  ...
  default void drawEntityPoint(final RendererAccess renderer,
      Point cameraTranslation, Entity entity) {
