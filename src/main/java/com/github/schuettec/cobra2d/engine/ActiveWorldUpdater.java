...
public class ActiveWorldUpdater {
			...
			while (running) {
				...
				if (doMapUpdate) {
					...
					while ((excess > period) && (skips < ActiveWorldUpdater.this.maxFrameSkips)) {
						...
						if (doMapUpdate) {
							map.update(fps, deltaTime); // update
							mapUpdates++;
						}
	...
	public ActiveWorldUpdater(int fps, boolean doMapUpdate, boolean doRender, Cobra2DWorld map, Renderer renderer) {
		super();
		this.fps = fps;
