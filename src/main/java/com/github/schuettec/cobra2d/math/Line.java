...
/**
 * This class describes a geometric line in a 2D coordinate
 * system. It is
 * basically a representation of the formula
 * <code>g(x) = m * x + b</code>
 *
 * @author Chris
 *
...
public class Line implements Shape, Cloneable {
   ...
   * @param l
   *        The line.
   * @return Returns the intersection represented by
   *         {@link Point} if this line
   *         intersects the specified line. Otherwise
   *         <code>null</code> is
   *         returned.
   */
  public Point intersects(final Line l) {
    ...
    // 2. Fall | --
    if (this.parallelY) {
      final Point schnittPunkt = new Point(this.x1.x,
          l.getValue(this.x1.x));
      if (this.isDefined(schnittPunkt)
          && l.isDefined(schnittPunkt)) {
        return new Point(schnittPunkt.getX(),
            schnittPunkt.getY());
      } else {
        return null;
      }
    ...
    // 3. Fall -- |
    if (l.isParallelY()) {
      final Point schnittPunkt = new Point(l.getX1().x,
          this.getValue(l.getX1().x));
      if (l.isDefined(schnittPunkt)
          && this.isDefined(schnittPunkt)) {
        return new Point(schnittPunkt.getX(),
            schnittPunkt.getY());
      } else {
        return null;
  ...
  public boolean isDefined(final Point point) {
    if (this.parallelY) {
      ...
      final double ymin = Math.min(this.getX1().y,
          this.getX2().y);
      final double ymax = Math.max(this.getX1().y,
          this.getX2().y);
    ...
    if (point.x >= xmin && point.x <= xmax) {
      // Beim folgenden Vergleich, kommt es vor, dass die zwei
      // letzten
      // Nachkomma stellen nicht gleich sind. Das liegt an der
      // Errechnung
      ...
      // Korrekte Aussage, allerdings hatte hier zuvor eine
      // Kollision
      // stattgefunden
      // Lösungsansatz: Auf ungenauen Integer runden und auf
      // Gleichheit
      // prüfen
      if (Math2D.saveRound(this.getValue(point.x)) == Math2D
          .saveRound(point.y)) {
        return true;
      }
    }
  ...
  public Line rotate(double degrees) {
    ...
    Point newX1 = Math2D.getCircle(center,
        Math2D.getEntfernung(x1, center), degrees);
    Point newX2 = Math2D.getCircle(center,
        Math2D.getEntfernung(x2, center), degrees);
  ...
  public Line scale(double scaleFactor) {
    ...
    Point newX1 = Math2D.getCircle(center,
        Math2D.getEntfernung(x1, center), angle);
    Point newX2 = Math2D.getCircle(center,
        Math2D.getEntfernung(x2, center), angle + 180);
    x1.setLocation(newX1);
    x2.setLocation(newX2);
    return this;
  }
  ...
  public double getMinX() {
    return Math.min(this.getX1().x, this.getX2().x);
  }

  public double getMaxX() {
    return Math.max(this.getX1().x, this.getX2().x);
  }
}
