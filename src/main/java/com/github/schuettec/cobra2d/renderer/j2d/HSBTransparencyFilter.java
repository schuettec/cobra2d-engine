...
public class HSBTransparencyFilter extends RGBImageFilter {
	...
	public HSBTransparencyFilter(final int hue, final int maxHueDistance, final int minSaturation,
	...
	public int filterRGB(final int x, final int y, final int rgb) {
		...
		final float[] hsbSrc = Color.RGBtoHSB(pixel.getRed(), pixel.getGreen(), pixel.getBlue(), null);
		...
		if (hueSrc < (this.hueTrans - this.maxHueDistance) || hueSrc > (this.hueTrans + this.maxHueDistance)) {
			return rgb;
			...
			int alpha = 255;
			alpha = Math.round((255f / this.maxHueDistance) * Math.abs(distance));
			...
			final Color newColor = new Color(Color.HSBtoRGB(targetHue, satSrc, brgSrc));

			return new Color(newColor.getRed(), newColor.getGreen(), newColor.getBlue(), alpha).getRGB();
		}
	}
}
