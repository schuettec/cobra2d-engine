...
public class HueBrightnessTransparencyFilter extends RGBImageFilter {
	...
	public HueBrightnessTransparencyFilter(final int hue, final float maxBrightness) {
	...
	public int filterRGB(final int x, final int y, final int rgb) {
		...
		final float[] hsbSrc = Color.RGBtoHSB(srcC.getRed(), srcC.getGreen(), srcC.getBlue(), null);

		final float hueSrc = hsbSrc[0];
		final float brgSrc = hsbSrc[2];

		if (hueSrc >= (this.hueTrans - 0.13f) && hueSrc <= (this.hueTrans + 0.1f)) {
		...
		return new Color(srcC.getRed(), srcC.getGreen(), srcC.getBlue(), alpha).getRGB();
